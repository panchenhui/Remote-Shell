...
char* change_file_mode(char* filename, char* mode){
	...
	}else if(strcmp(mode, "other_read") == 0){
	...
	}else if(strcmp(mode, "other_write") == 0){
...
int main(int argc , char *argv[])
{
    ...
    char commands[2000];
    char line[2000];
    int end = 0, invalid_command = 0, sendLen = 0;

    // Varibles for 'ls' command
    FILE *pipein_fp, *pipeout_fp;
    FILE *command_log = fopen("log.txt", "w");
    char sendBuff[1024], readBuff[1024];
	...
	// list files under the current directory
		...
		int i = 0;
		/* Processing loop */
		while(fgets(sendBuff, 1024, pipein_fp)){
	   		fputs(sendBuff, command_log);
			send(csock, sendBuff, sizeof(sendBuff), 0);
		}
		//send results back to client
            	//sendLen = strlen(sendBuff);
		//send(csock, &sendLen, sizeof(sendLen), 0);
		//send(csock, sendBuff, sendLen, 0);
	...
	// print working directory
		...
		int i = 0;
		/* Processing loop */
		while(fgets(readBuff, 1024, pipein_fp)){
	   		fputs(readBuff, command_log);
